...
function App() {
	...
	const [hint, setHint] = useState(
		' "the one and the only who developed this game"'
	);
	const [secretWord, setSecretWord] = useState(["A", "D", "I", "R", "A"]);
	const [lettersLeft, setLetterLeft] = useState(secretWord.length);
	const [GuessedWord, setGuessedWord] = useState(["_","_","_","_","_"]);
	...
	return (
		<div className="App">
			...
			{lettersLeft == 0 ? (
				<>
					{" "}
					<Win /> <Characters word={GuessedWord} />
				</>
			) : remainGuesses == 0 ? (
				<Gameover />
			) : (
